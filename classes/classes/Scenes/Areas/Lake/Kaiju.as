...
private function insultTheKaijuFirstMeeting():void {
	...
	if(kaijuCock()) {
		...
		if(player.hasStatusAffect("Knows Whitefire") >= 0 && (player.hasPerk("Blood Mage") < 0 && player.fatigue + kGAMECLASS.spellCost(30) > 100)) {
...
private function repeatKaijuEncounter():void {
	...
	if(kGAMECLASS.itemCount("IncubiD") + kGAMECLASS.itemCount("P.Draft") >= 10 && !kaijuCock()) {
...
private function badEndPartTwo():void {
	...
	if(kaijuCock()) {
		...
		outputText("\n\n\"<i>Do you feel it working yet?  Can you guess what it did?  I’ll give you a hint,</i>\" the giantess says, a mad look of lust in her eyes as she gazes at your nude, helpless form.  It appears the turtle girl you know is gone, completely taken over by her libido.  She " + assholeOffLimits("lifts you up", "turns you around") + " and you suddenly feel the giant head of her dick resting on your " + assholeOffLimits("mound", "[butt]") + ".  Why is she rubbing your " + assholeOffLimits("cunt", "ass") + " with her dick, she knows it won’t fit right?  It’s bigger and wider than your whole body!");
		outputText("\n\nAnd that’s when you feel the impossible.  Your " + assholeOffLimits("[vagina]", "asshole") + " expands, gently giving ground to the leviathan at your " + assholeOffLimits("entrance", "backdoor") + " as you feel your hips part and stretch away from each other.  Her magic pink cum has turned you into the most elastic fuck toy in the land!  Oh, this is just too wrong.");
		if(assholeOffLimits()) {
		...
		outputText("\n\nSuddenly her entire cockhead enters your " + assholeOffLimits("cunt", "ass") + ", stretching you out obscenely.  You orgasm instantly.  You might handle " + assholeOffLimits("cock", "anal") + " better than this under normal conditions, but then there’s nothing normal about taking something this huge.");
	...
	outputText("\n\nThe first planty-cock hits your ass like a switch, spanking your [butt] till your cheeks are blushing red before the tip snuggles up to your puckered sphincter" + assholeOffLimits(" and tears out the plug within it") + ".  It wriggles wildly, working itself up in ecstatic anticipation, or at least the plant-like version of anticipation, drippy vine-pre-seed lubing up your hole before pushing forth with all its might, meeting only a little resistance before the pressure and fluid mixture eases it into your [butt].");
